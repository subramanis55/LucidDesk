...
namespace DeskDS.DeskStructure
{
    public class DeskConnectionInformation
    {
        ...
        public void AddDeskScreensInformations(List<Screen> screens)
        {
            foreach (Screen screen in screens)
            {
                ScreenInformation.AddScreen(screen);
            }
            //foreach (var screen in System.Windows.Forms.Screen.AllScreens)
            //{
            //    ScreenInformation.AddScreen(new Screen() { Bounds = screen.Bounds, IsPrimaryScreen = screen.Primary, DeviceName = screen.DeviceName });
            //}
        }
    }
}
