...
namespace DeskBackend.Database
{
    public static class ServerDatabaseManager
    {
        ...
        public static bool CreateDeskProfile(Desk deskProfile)
        {
            try
            {
                ...
                {
                    string CreateDeskProfileQuery = $"Insert into  DeskProfile(IPAddress,HostName,ProfileName,ProfileImage,DesktopImage,Password,MacAddress,OsName,PcName,RecentLoginTime)   values('{deskProfile.IPAddress}','{deskProfile.HostName}','{deskProfile.ProfileName}','{Convert.ToBase64String( deskProfile.ProfileImage)}','{Convert.ToBase64String(deskProfile.DesktopImage)}','{deskProfile.Password}','{deskProfile.MacAddress}','{deskProfile.OsName}','{deskProfile.PcName}','{deskProfile.RecentLoginTime.ToString("yyyy-MM-dd HH:mm:ss")}')";
        ...
        public static Desk GetDeskProfile(string macAddress)
        {
            ...
            if (dataTable.Rows == null || dataTable.Rows.Count == 0)
                return null;
            Desk desk = new Desk() { IPAddress = Convert.ToString(dataTable.Rows[0]["IPAddress"]), IsFavorite = Convert.ToBoolean(dataTable.Rows[0]["IsFavorite"]), HostName = Convert.ToString(dataTable.Rows[0]["HostName"]), ProfileName = Convert.ToString(dataTable.Rows[0]["ProfileName"]), ProfileImage = Convert.FromBase64String(dataTable.Rows[0]["ProfileImage"].ToString()), DesktopImage = Convert.FromBase64String(dataTable.Rows[0]["DesktopImage"].ToString()), Password = Convert.ToString(dataTable.Rows[0]["Password"]), MacAddress = Convert.ToString(dataTable.Rows[0]["MacAddress"]), OsName = Convert.ToString(dataTable.Rows[0]["OsName"]), PcName = Convert.ToString(dataTable.Rows[0]["PcName"]), RecentLoginTime = (DateTime)dataTable.Rows[0]["PcName"] };
            return desk;
        }
